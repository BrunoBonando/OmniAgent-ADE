...
<!-- GENERATED from CLAUDE.md — promoted as authoritative by sync script. Edit CLAUDE.md to change. -->
...
- Monorepo: Rust/Cargo workspace + a native macOS app:
  ...
  - `crates/omniagent-pty-daemon` — the persistent PTY daemon: sole owner of real PTYs/child processes, versioned socket protocol the native app speaks (`SessionConnection.swift`)
    - **Remote environment sharing** (`docs/superpowers/specs/2026-09-01-remote-environment-sharing-design.md`; supersedes the 2026-08-30 and 2026-08-31 phase specs, whose per-workspace model is gone): sharing is **machine-wide**, not per-workspace. One settings row `remote_sharing` = `{"enabled": bool}` is the switch, reached from Settings › Remote and the menu bar (icon: template off, green sharing, blue being driven, purple driving another Mac). `src/relay.rs` holds one outbound control WebSocket to `relay.omni-agent.ai` (staging `.dev`) **iff** three things hold (`sharing_should_be_live`, `src/server.rs`): the switch is on, a `relay_device_token` row exists, and **at least one local app connection is attached** (5 s grace). That third condition is not cosmetic — it is what makes remote-access **chaining structurally impossible**, because a Mac driving another has swapped its single `SessionConnection` away from its own daemon and so refuses everyone inbound. `PROTOCOL_VERSION` is **2**; a skewed peer is told (`RefusalCode`, machine-readable, with display text) rather than left in a reconnect loop. Remote clients hold an **exclusive lease** (one at a time; the same viewer reclaims its own after a blip) and are confined to an explicit allowlist in `authorize_remote` — session, pane, layout, `Roots*`, `Brain*` and `ListDirectory` (0x1d, names and is-dir flags only; **there is no file-read RPC in this system and there must not be one**). Deny-by-default still holds: an exhaustive `MessageKind` match fails to COMPILE until a new kind is classified. `GetSetting`/`SetSetting` are refused on the protected keys `remote_sharing`, `relay_device_token`, `remote_control_blocked`, `engine_search_path` and any `auth_*` — on **both** arms. **Whoever drives owns the grid**: the viewer sends `Resize` so terminals reflow at its own resolution, the host suppresses its own while shared (`sharingIsLive` gating `TerminalSurfaceView.flushResize`), and `SessionResized` is deleted. `PublishHostState` (0x1c, local-only) → `HostState` (0x8e) carries what only the host app knows — CPU/memory/GPU gauges, Claude limits, engine availability — as opaque JSON the daemon never parses. `RemoteActivity` (0x8f, local-only) carries an activity log the daemon derives from frames it actually received — never from anything the viewer says about itself — appended as JSONL to `<data root>/remote-activity.jsonl` and rendered in an undismissable liquid-glass **takeover panel** on the host, with Terminate (`block:false`) and Block (`block:true`). The relay (`OmniAgent-Core/omniagent/relay/`) stays a dumb byte pipe but now **asserts the viewer's identity** in its `open` message (`user_sub`, `account_email`, `ip`/`country` from Cloudflare edge headers, `client`); the daemon refuses any connection whose asserted account does not both hash to the account directory it is serving and match that directory's `auth_account_email` row — a second check, in a second process, on a second fact. Absent fields are omitted, never placeholdered, and only relay-asserted rows are marked verified in the panel (`client` is a User-Agent and is not). The settings rows `remote_sharing`, `relay_device_token`, `remote_control_blocked` and `engine_search_path` are a JSON contract between Swift (writer) and Rust (reader) — snake_case keys, exact shapes in the spec and `SettingsKeys.swift`; never rename on one side alone.
  - `macos/` — native macOS app (`OmniAgent.xcodeproj`): AppKit primary workspace, SwiftUI for low-frequency surfaces (settings/onboarding/usage/inspectors), SwiftTerm for terminals; see "Native macOS app (macos/)" above
    ...
    - **Flow-style layout** (2026-09-01, `docs/superpowers/specs/2026-09-01-flow-layout-design.md`): every destination sits inside one inset `ContentCardView` — 12pt off the content column's leading, trailing and bottom edges, under the 38pt title strip — rather than filling the column; since 2026-09-02 the card is tinted Liquid Glass (`WorkspaceGlass.sheet` under a `ShellGlassTintView` wearing `ShellPalette.contentCardTint`, the token a later theme swaps; flat fill plus hairline below macOS 26). Since 2026-09-02 the window is **one ground**: the `PaneGroundView` blue gradient (`44, 52, 87` → `29, 32, 52`) is the window's content view behind the split, the sidebar paints nothing of its own (no glass sheet, wash or trailing hairline — those stay on the review panel only), the content column is transparent, and the split divider is `GroundSplitView`'s clear one, so the card is the only thing floating on the sheet. Home and Insights wear `PageShellView` (`PageShell.swift`: H1 title → optional underline tab strip → hairline rule → scrolling body); a shell built with no tabs carries no underline at all. `WorkspaceDestination.insights` is a real page (`InsightsView.swift`): **Usage** is three KPI cards over the hosted SwiftUI `UsageView(embedded:)`, **Activity** is a second `ReviewPanelInsightsView` over *every* terminal pane in the window — the review panel's own tab and this one are both built by the controller's shared `insightsLanes` helper, and the page states the tape's height from the lane count (`InsightsSurfaceView.applyActivity`) because the tape lays its lanes out in manual frames. The title bar gained the account avatar and the notifications bell at its trailing edge, each opening a popover — the bell a `HomeDropdown` row list (`showNotifications`, the feed capped at 20 entries), the avatar an `AccountMenuView` (`showAccountMenu`: avatar, name, email, the Free plan with an Upgrade pill, Manage account — and deliberately no Log out, which stays under Settings › Accounts); the sidebar's account row was replaced by **Settings** (badged) + **Help** rows, and the Settings panel's "offered" mode was deleted with it.
