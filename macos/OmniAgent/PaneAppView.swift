...
final class PaneAppView: NSView {
    ...
    let composerField: HomeComposerField = {
        ...
        // Wraps and grows. The PTY used to take one line per send, which is
        // what kept this single-line; `TerminalSurfaceView.framed` wraps a
        // multi-line draft in bracketed paste now, so the newlines reach the
        // TUI as newlines instead of as submits.
        field.allowMultipleLines()
        field.translatesAutoresizingMaskIntoConstraints = false
        return field
    }()

    /// The full-pane dim behind an expanded composer. Below `composerGlass`
    /// and above everything else, so the transcript recedes and the card
    /// being typed into is the only lit thing in the pane.
    ///
    /// Built once and kept hidden rather than made on demand: it carries a
    /// `WorkspaceGlass` sheet, and constructing one mid-animation is a
    /// visible hitch.
    private let expandScrim = PaneAppExpandScrimView()

    /// Whether the composer is in its expanded state. Internal rather than
    /// `private` so the tests can read it without going through the button.
    private(set) var isComposerExpanded = false

    /// Grows the composer to most of the pane and back. Internal rather than
    /// `private` so a test can press it rather than call the action directly
    /// — pressing is what proves it is wired.
    let expandButton = PaneAppView.composerButton(
        symbol: "arrow.up.left.and.arrow.down.right",
        accessibility: "Expand the composer"
    )

    /// The field's own height, driven by content while collapsed and by the
    /// pane while expanded. Assigned in `init`, so implicitly unwrapped
    /// rather than optional at every use.
    private var composerFieldHeight: NSLayoutConstraint!
    ...
    init(sessionID: String, cwd: String, home: URL = ClaudeModel.homeDirectory) {
        ...
        expandScrim.onClick = { [weak self] in
            self?.setComposerExpanded(false)
        }
        ...
        expandButton.action = #selector(toggleComposerExpanded)

        let controls = NSStackView(views: [attachButton, expandButton, NSView(), sendButton])
        ...
        // Ordered: the scrim covers the transcript, the glass sits over the
        // scrim. `expandScrim` before `composerGlass` in this list is the
        // whole of that z-order.
        for view in [scrollView, emptyStateLabel, expandScrim, composerGlass] as [NSView] {
            addSubview(view)
        }
        ...
        NSLayoutConstraint.activate([
            ...
            expandScrim.topAnchor.constraint(equalTo: topAnchor),
            expandScrim.bottomAnchor.constraint(equalTo: bottomAnchor),
            expandScrim.leadingAnchor.constraint(equalTo: leadingAnchor),
            expandScrim.trailingAnchor.constraint(equalTo: trailingAnchor),
        ...
        // The field's height is authored rather than intrinsic: a wrapping
        // `NSTextField` grows without bound, and the whole point of the cap
        // is that a long draft scrolls inside the card instead of pushing
        // the card up over the transcript.
        composerFieldHeight = composerField.heightAnchor.constraint(
            equalToConstant: composerField.fittingHeight(forWidth: Self.transcriptColumnWidth - 28)
        )
        composerFieldHeight.isActive = true
        ...
        // report a width. Raising only the transcript would leave that trap
        // armed.
        for column in [messageStack, composerGlass] as [NSView] {
    ...
    override func layout() {
        ...
        // After `super.layout()`, so the field's width is the resolved one
        // rather than whatever it was before this pass.
        updateComposerHeight()
        // Top inset is zero now that the stats bar is gone: nothing floats
        // over the head of the transcript, so the first message wants the
        // pane's own top edge rather than clearance under a strip.
        guard scrollView.contentInsets.bottom != clearance
            || abs(scrollView.contentInsets.top) > 0.5
        else { return }
        ...
        scrollView.scrollerInsets = NSEdgeInsets(
            top: 0, left: 0, bottom: -clearance, right: 0
        )
    }
    ...
    /// columns — the transcript (`messageStack`) and the composer
    /// (`composerGlass`) — so they line up.
    ...
    /// Internal rather than `private` so a test can send without
    /// synthesising a click on the button.
    @objc func submitComposer() {
        let text = composerField.stringValue.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else { return }
        onSubmit?(text)
        composerField.stringValue = ""
        // What earns the next scheduled `/usage` fetch. Sending is the only
        // thing that can move those numbers, so it is the only thing that
        // makes re-reading them worth a request.
        ClaudeUsageLimitsPoller.shared.noteActivity()
        // An expanded composer has done its job once the long draft is gone.
        if isComposerExpanded { setComposerExpanded(false) }
        updateComposerHeight()
    }

    // MARK: - Composer size

    /// The tallest the field grows on its own, in points — about six lines at
    /// 14pt. Past this a draft scrolls inside the field rather than pushing
    /// the card further up over the transcript; the expand button is the way
    /// to see more of it at once.
    private static let collapsedFieldCap: CGFloat = 116

    /// The share of the pane an expanded composer takes.
    private static let expandedPaneShare: CGFloat = 0.6

    /// Everything in the card that is not the field: the 22pt inset above it,
    /// the 18pt gap below, the 26pt controls row and the 24pt bottom inset.
    private static let composerChromeHeight: CGFloat = 22 + 18 + 26 + 24

    /// Sizes the field: to its content while collapsed, to the pane while
    /// expanded.
    ///
    /// Called on every keystroke and on every resize, so it returns early
    /// when nothing moved — an unchanged constant still triggers a layout
    /// pass, and a pass per keystroke is visible on a long draft.
    func updateComposerHeight() {
        let width = max(composerField.bounds.width, 1)
        let target: CGFloat
        if isComposerExpanded {
            target = expandedFieldHeight()
        } else {
            target = min(composerField.fittingHeight(forWidth: width), Self.collapsedFieldCap)
        }
        guard abs(composerFieldHeight.constant - target) > 0.5 else { return }
        composerFieldHeight.constant = target
        needsLayout = true
    }

    /// The field's height in the expanded state, floored at the collapsed cap
    /// so "expand" never makes the composer *smaller* — which is what a share
    /// of a short pane would otherwise work out to.
    private func expandedFieldHeight() -> CGFloat {
        max(bounds.height * Self.expandedPaneShare - Self.composerChromeHeight, Self.collapsedFieldCap)
    }

    @objc private func toggleComposerExpanded() {
        setComposerExpanded(!isComposerExpanded)
    }

    /// Internal rather than `private` so a test can drive both directions
    /// without synthesising two button presses.
    func setComposerExpanded(_ expanded: Bool) {
        guard expanded != isComposerExpanded else { return }
        isComposerExpanded = expanded
        expandButton.image = NSImage(
            systemSymbolName: expanded
                ? "arrow.down.right.and.arrow.up.left"
                : "arrow.up.left.and.arrow.down.right",
            accessibilityDescription: expanded ? "Collapse the composer" : "Expand the composer"
        )
        expandScrim.setVisible(expanded)
        updateComposerHeight()
        // Expanding is a request to write, so the caret goes where the writing
        // happens without a second click.
        if expanded { focusComposerAtEndOfDraft() }
    }
...
/// The full-pane dim behind an expanded composer.
///
/// A `WorkspaceGlass` sheet where the platform has one and a flat scrim
/// otherwise, fading rather than snapping — the composer growing under a
/// backdrop that appeared instantly reads as two unrelated events.
final class PaneAppExpandScrimView: NSView {
    /// How long the dim takes to arrive or leave. Matched to the composer's
    /// own growth so the two read as one movement.
    ...
    override init(frame frameRect: NSRect) {
        ...
        isHidden = true
        alphaValue = 0
        if let glass = WorkspaceGlass.sheet(cornerRadius: 0) {
            glass.translatesAutoresizingMaskIntoConstraints = false
            addSubview(glass)
            NSLayoutConstraint.activate([
            ...
            layer?.backgroundColor = NSColor(white: 0, alpha: 0.45).cgColor
        }
        // Swallows clicks aimed at the transcript behind it: while the
        // composer is expanded the transcript is backdrop, and a click that
        // fell through to it would select text nobody can see properly.
    ...
    /// Called for a click on the dim rather than on the composer.
    var onClick: (() -> Void)?

    /// A click on the backdrop is a click *away* from the composer, which is
    /// the idiom focus mode already uses here: esc, or a click outside.
    /// Swallowed either way — it must never reach the transcript and select
    /// text nobody can properly see.
    override func mouseDown(with event: NSEvent) {
        onClick?()
    }

    /// Fades in or out, landing instantly under Reduce Motion — the same rule
    /// every other animation in this view follows.
    func setVisible(_ visible: Bool) {
        ...
        NSAnimationContext.runAnimationGroup { context in
            ...
            guard let self else { return }
            // Re-read rather than captured: a fast toggle can land this
            // completion after the *next* transition started, and hiding on a
            // stale value would blank a scrim that is meant to be up.
            if self.alphaValue == 0 { self.isHidden = true }
        }
    }
}

extension PaneAppView: NSTextFieldDelegate {
    /// Grows the card as the draft wraps.
    func controlTextDidChange(_ notification: Notification) {
        updateComposerHeight()
    }

    /// Return submits; Shift+Return breaks the line; Escape collapses.
    ///
    /// AppKit maps *both* Return and Shift+Return to `insertNewline:`, so the
    /// modifier has to be read off the event that caused the selector — there
    /// is no separate command for the shifted one. (`insertNewlineIgnoringFieldEditor:`
    /// is Option+Return's, which keeps working and is left alone.)
    func control(
        _ control: NSControl,
        textView: NSTextView,
        ...
        if commandSelector == #selector(NSResponder.cancelOperation(_:)), isComposerExpanded {
